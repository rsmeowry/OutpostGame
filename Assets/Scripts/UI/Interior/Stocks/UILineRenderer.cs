// https://github.com/Radishmouse22/UILineRenderer/blob/main/UILineRenderer.cs
...
namespace UI.Interior.Stocks
{
    ...
    public class UILineRenderer : MaskableGraphic
    {
        ...
        public (string, string)[] circlePoints;
        ...
        [ContextMenu("Util/Reposition circles")]
        public void RepositionCircles()
        {
            ...
            for (var i = 0; i < points.Length; i++)
            {
                ...
                var pf = (RectTransform) Instantiate(circlePrefab, transform).transform;
                var p = points[i];
                p.x -= rectTransform.rect.width / 2;
                p.y -= rectTransform.rect.height / 2;
                pf.anchoredPosition = p;
                var pv = pf.localPosition;
                pv.z -= 2;
                pf.localPosition = pv;
                var singlePoint = pf.GetComponent<SingleStocksPoint>();
                singlePoint.title = point.Item1;
                singlePoint.desc = point.Item2;
            }
        }
